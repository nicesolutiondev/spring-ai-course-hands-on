...
특정 클래스(또는 인터페이스) 구현을 요구받으면, 코드를 작성하기 전에 항상 아래 순서를 따른다.

1. **`docs/03-구현.md`** 에서 해당 클래스에 대한 항목(역할·입력·출력·확인 방법·주의 사항)을 찾아 확인한다.
2. 해당 클래스의 **Javadoc**, 그리고 그 클래스가 구현하는 **인터페이스의 Javadoc** 을 읽고
...
이 절차는 이후 모든 "구현해줘" 요청에 매번 적용한다.

## solution 브랜치를 보지 않는다
...
그리고 **`docs/03-구현.md`** 의 해당 카드다. 그 둘에 없는 것은 설계 판단에 맡긴 부분이다.

`solution` 은 실습이 끝난 뒤 자기 구현과 견주어 보는 용도다.
